void handleRoot() {
  ...
  if (espConfig.statuses.authenticated) {
    Serial.println("valaszol login utan");
...
void handleWifiSettings() {
  ...
  if (espConfig.statuses.authenticated) {
    ...
    responseString += String("<br><h4>Not Logged In</h4><br></body></html>");
  }
...
void handleNodeInfo() {
  espConfig.getContentSerd(espConfig.serContent);
  Serial.println(espConfig.serContent);
  webserver.sendHeader("Cache-Control", "no-cache, no-store, must-revalidate");
  webserver.sendHeader("Pragma", "no-cache");
  webserver.sendHeader("Expires", "-1");
  ...
  webserver.sendContent(espConfig.serContent);
