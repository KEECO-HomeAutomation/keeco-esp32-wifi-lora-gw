void initWifiOnBoot() {
  int wifiConnWait = 0;
  sprintf(espConfig.wifiAP.ssid, "KEECO_AP_%06X", (uint16_t)ESP.getEfuseMac());
  WiFi.mode(WIFI_STA);
  WiFi.onEvent(stopAPSrvCallback, WiFiEvent_t::SYSTEM_EVENT_STA_GOT_IP);
  WiFi.onEvent(startAPSrvCallback, WiFiEvent_t::SYSTEM_EVENT_STA_DISCONNECTED);
  WiFi.setAutoReconnect(true);
  WiFi.setHostname("KKECO_HW_Node");
